...
export default function App() {
  ...
  const atualizarGraficosResumo = () => {
    ...
    const criarPizza = (ctx, dados, instanciaRef) => {
      ...
      if (Object.keys(dados).length === 0) {
        instanciaRef.current = new Chart(ctx, { type: "pie", data: { labels: ["Sem dados"], datasets: [{ data: [1], backgroundColor: ["#ccc"] }] }, options: { plugins: { legend: { position: "bottom" } } } });
        return;
      }
      const labels = Object.keys(dados);
      const bgColors = labels.map((_, i) => coresCategoria[i % coresCategoria.length]);
      instanciaRef.current = new Chart(ctx, { type: "pie", data: { labels, datasets: [{ data: Object.values(dados).map((v) => +v.toFixed(2)), backgroundColor: bgColors }] }, options: { plugins: { legend: { position: "bottom" } }, responsive: true } });
    };
  ...
  const calcularSaldo = (mesAtual, anoAtual) => {
    const entradas = transacoes.filter((t) => t.mes === mesAtual && t.ano === anoAtual && t.tipo === "entrada").reduce((acc, cur) => acc + cur.valor, 0);
    const saidas = transacoes.filter((t) => t.mes === mesAtual && t.ano === anoAtual && t.tipo === "saida").reduce((acc, cur) => acc + cur.valor, 0);
  ...
  const adicionarEntrada = (e) => {
    ...
    if (recorrenteEnt) {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        novas.push({ descricao: descricaoEnt + " (recorrente)", valor: valNum, categoria: categoriaEnt, mes: meses[mesIndex], ano: ano, tipo: "entrada", id: Date.now() + i });
      }
    } else {
      novas.push({ descricao: descricaoEnt, valor: valNum, categoria: categoriaEnt, mes: mesEnt, ano: new Date().getFullYear(), tipo: "entrada", id: Date.now() });
    }
    setTransacoes(novas);
    setDescricaoEnt(""); setValorEnt(""); setCategoriaEnt(categorias[0] || ""); setMesEnt(meses[new Date().getMonth()]); setRecorrenteEnt(false); setPagina("resumo");
  };
  ...
  return (<div>Seu App com botões de excluir aqui</div>);
}
